...
static uint64_t usg_tsc_resolution_hz;

static unsigned int
usg_cpu_get_model(uint32_t fam_mod_step)
{
...
#define NS_PER_SEC 1E9
	...
	if (clock_gettime(CLOCK_MONOTONIC_RAW, &t_start) == 0) {
		uint64_t ns, end, start = usg_rdtsc();
...
{
	uint64_t start = usg_rdtsc();
	sleep(1);
	return usg_rdtsc() - start;
}

uint64_t
usg_get_tsc_hz(void)
{
	return usg_tsc_resolution_hz;
}

uint64_t
usg_get_futurn_tsc(uint64_t ms)
{
	uint64_t inc_tsc = (usg_tsc_resolution_hz / 1000) * ms;

    return (usg_rdtsc() + inc_tsc);
}
